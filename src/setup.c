...
void setup() {
  ...
  room_generator(
      &current_room_array, 0,
      1); // generating the starting room array with no entry and exit up
  bullets_list = NULL;
  hero.hp = 5;
  hero.active = true;
...
//////////////////////////////////////////////////////////////////////////////
  ...
  for (int i = 0; i < num_enemies; i++) {
    if (enemies[i].active && enemies[i].type == 0) {
      SDL_Rect enemy_rect = {enemies[i].x, enemies[i].y, enemies[i].width,
                             enemies[i].height};
  ...
  if (difftime(time(NULL), last_attack_time) < 1.0) {
    ...
    SDL_SetTextureColorMod(hero.texture, 255, 0,
                           0); // Устанавливаем красный цвет
  } else {
  ...
  for (; bullet != NULL;) {
    SDL_Rect bullet_rect = {bullet->x, bullet->y, bullet->width,
                            bullet->height};
  ...
  while (bullet != NULL) {
    if (bullet->active) {
      ...
      for (int i = 0; i < num_enemies; ++i) {
        if (enemies[i].active) {
          SDL_Rect enemy_rect = {enemies[i].x, enemies[i].y, ENEMY_WIDTH,
                                 ENEMY_HEIGHT};
...
void update() {
  ...
  int is_object = is_next_position_object(
      hero.width, hero.height, hero.x - hero.xspeed * delta_time,
  ...
  for (int i = 0; i < num_enemies; i++) {
    update_enemy_position(&enemies[i], &hero, ENEMY_SPEED, delta_time, enemies,
                          num_enemies);
  }
}
